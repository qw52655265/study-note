<template>
  <div class="flex column stretch auto">
    <!-- <script
      ref="script"
      :name="name"
      type="text/plain"
      class="flex column stretch auto"
    /> -->
  </div>
</template>

<script>
// import LoadEvent from '@/instanceMethods/Event.js'
// import Debounce from '@/instanceMethods/Debounce.js'
...
export default {
  ...
  // },
  // // created() {
  // //   this.defaultConfig.UEDITOR_HOME_URL =
  // //     (process.env.NODE_ENV === 'production' ? '/guiderank-admin' : '') +
  // //     this.defaultConfig.UEDITOR_HOME_URL
  // //     console.log(this.defaultConfig.UEDITOR_HOME_URL)
  // // },
  // methods: {
  ...
  //   // 实例化编辑器
  //   _initEditor() {
  //     // 这么做是为了支持 Vue SSR，因为如果把 id 属性放在 data 里会导致服务端和客户端分别计算该属性的值，而造成 id 不匹配无法初始化的 BUG
  //     this.$refs.script.id = this.id = `editor_${Math.random()
  //       .toString(16)
  //       .slice(-6)}`
  //     this.init()
